...
@cli.command(name='combine-roadmaps', help='Combine a structured YAML roadmap and an unstructured Markdown roadmap into a single Markdown file.')
@click.option('--yaml-file', type=click.Path(exists=True), default='GITSCAFFOLD_ROADMAP.yml', show_default=True, help='Path to the structured YAML roadmap file.')
@click.option('--md-file', type=click.Path(), default='ROADMAP.md', show_default=True, help='Path to an existing unstructured Markdown roadmap file to append.')
@click.option('--output-file', type=click.Path(dir_okay=False, writable=True), default='COMBINED_ROADMAP.md', show_default=True, help='Path for the combined output Markdown file.')
def combine_roadmaps(yaml_file, md_file, output_file):
    """Combines a YAML roadmap and a Markdown roadmap into a single, comprehensive Markdown file."""
    click.secho("=== Combining Roadmaps ===", fg="bright_blue", bold=True)
    
    # 1. Parse and validate the YAML roadmap
    try:
        raw_roadmap_data = parse_roadmap(yaml_file)
        validated_roadmap = validate_roadmap(raw_roadmap_data)
        click.secho(f"Successfully parsed YAML roadmap: {validated_roadmap.name}", fg="green")
    except Exception as e:
        click.secho(f"Error parsing YAML file '{yaml_file}': {e}", fg="red", err=True)
        sys.exit(1)
        
    # 2. Generate Markdown from the YAML roadmap data
    md_parts = []
    md_parts.append(f"# {validated_roadmap.name}\n")
    if validated_roadmap.description:
        md_parts.append(f"{validated_roadmap.description}\n")

    if validated_roadmap.milestones:
        md_parts.append("## Milestones\n")
        for m in validated_roadmap.milestones:
            due_date_str = f" (due: {m.due_date})" if m.due_date else ""
            md_parts.append(f"- {m.name}{due_date_str}")
        md_parts.append("\n")

    if validated_roadmap.features:
        md_parts.append("## Features\n")
        for feat in validated_roadmap.features:
            md_parts.append(f"### {feat.title}\n")
            if feat.description:
                md_parts.append(f"**Description:** {feat.description}\n")
            if feat.milestone:
                md_parts.append(f"**Milestone:** {feat.milestone}\n")
            if feat.labels:
                labels_str = ", ".join([f"`{l}`" for l in feat.labels])
                md_parts.append(f"**Labels:** {labels_str}\n")
            if feat.assignees:
                assignees_str = ", ".join([f"`{a}`" for a in feat.assignees])
                md_parts.append(f"**Assignees:** {assignees_str}\n")
            
            if feat.tasks:
                md_parts.append("\n#### Tasks\n")
                for task in feat.tasks:
                    md_parts.append(f"- [ ] {task.title}")
            md_parts.append("\n")

    generated_md_content = "\n".join(md_parts)
    click.secho("Generated Markdown from YAML file.", fg="cyan")

    # 3. Read existing Markdown file if it exists
    existing_md_content = ""
    md_file_path = Path(md_file)
    if md_file_path.is_file():
        try:
            existing_md_content = md_file_path.read_text(encoding='utf-8')
            click.secho(f"Appending content from '{md_file_path}'.", fg="cyan")
        except Exception as e:
            click.secho(f"Warning: Could not read '{md_file_path}': {e}", fg="yellow")

    # 4. Combine and write to output file
    final_content = generated_md_content
    ...
    try:
        ...
        click.secho(f"Successfully combined roadmaps into '{output_path}'", fg="green", bold=True)
    except IOError as e:
        click.secho(f"Error writing to file '{output_path}': {e}", fg="red", err=True)
        sys.exit(1)
